...
void SimpleRenderSystem::createPipelineLayout() {
  ...
  if (vkCreatePipelineLayout(frgDevice.device(), &pipelineLayoutInfo, nullptr,
...
void SimpleRenderSystem::createPipeline(VkRenderPass renderPass) {
  ...
  frgPipeline = std::make_unique<FrgPipeline>(
      frgDevice, "shaders/triangle.vert.spv", "shaders/triangle.frag.spv",
      pipelineConfig);
}
...
void SimpleRenderSystem::renderGameObjects(
  ...
  for (auto &gameObject : gameObjects) {
    ...
    vkCmdBindDescriptorSets(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
                            ...
                            frgDescriptor.descriptorSet(), 0, nullptr);
