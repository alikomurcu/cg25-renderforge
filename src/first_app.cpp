...
void FirstApp::run() {
    ...
    simpleRenderSystem.getLightManager().addPointLight(glm::vec3(0.f, 0.f, 0.f),
    ...
    while (!frgWindow.shouldClose()) {
        ...
        cameraController.moveInPlaneXZ(frgWindow.getGLFWwindow(),
                                       frameTime,
                                       viewerObject);
...
void FirstApp::loadGameObjects() {
    ...
    for (const auto &path : file_paths) {
        ...
        std::cout << "# of vertices: " << g_obj.model->vertex_count()
                  << std::endl;
